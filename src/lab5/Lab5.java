...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener {
	...
	private void draw(GL2 gl2) {
		...
		// java -classpath
		// "E:\OpenGL-workspace\jogamp-all-platforms\jar\gluegen-rt.jar;E:\OpenGL-workspace\jogamp-all-platforms\jar\jogl-all.jar"
		// E:\OpenGL-workspace\lab5\src\lab5\Lab5.java
	...
	private void drawTriangle(GL2 gl2) {
		...
		gl2.glVertex2f(-1.2f, 0);
		gl2.glVertex2f(1.2f, 0);
		gl2.glVertex2f(0, 2.1f);
		gl2.glEnd();
	}

	private void drawPiramide(float size, GL2 gl2) {
		gl2.glPushMatrix();
		gl2.glScalef(size, size, size);

		gl2.glPushMatrix();
		gl2.glRotatef(90, 1, 0, 0);
		gl2.glRotatef(-18, 0, 0, 1);
		drawBase(gl2, 5);
		gl2.glPopMatrix();

		gl2.glPushMatrix();
		gl2.glTranslatef(0, 0, 1.62f);
		gl2.glRotatef(-51, 1, 0, 0);
		drawTriangle(gl2);
		gl2.glPopMatrix();

		gl2.glPushMatrix();
		gl2.glTranslatef(0, 0, -1f);
		gl2.glRotatef(35, 0, 1, 0);
		gl2.glTranslatef(0, 0, -0.8f);
		gl2.glTranslatef(-0.6f, 0, 0);
		gl2.glRotatef(51, 1, 0, 0);
		drawTriangle(gl2);
		gl2.glPopMatrix();
		...
		gl2.glPushMatrix();
		gl2.glTranslatef(0, 0, 1f);
		gl2.glRotatef(-72, 0, 1, 0);
		gl2.glTranslatef(0, 0, 1.3f);
		...
		gl2.glPushMatrix();
		gl2.glTranslatef(0, 0, -1f);
		gl2.glRotatef(70, 0, 1, 0);
		gl2.glTranslatef(0, 0, 1.95f);
		gl2.glTranslatef(-0.9f, 0, 0);
		gl2.glRotatef(-51, 1, 0, 0);
		drawTriangle(gl2);
		gl2.glPopMatrix();
	...
	private void drawBase(GL2 gl2, int numberOfSides) {
		gl2.glBegin(GL2.GL_POLYGON);
		float radius = 2f;
		double angle = 2 * Math.PI / numberOfSides;
		float[] coords = new float[numberOfSides * 2];

		int index = 0;
		for (int i = 0; i < numberOfSides; i++) {

			coords[index] = (float) (radius * Math.cos(i * angle));
			coords[index + 1] = (float) (radius * Math.sin(i * angle));
			index += 2;
		}
		for (int i = 0; i < numberOfSides; i++) {
			gl2.glVertex2fv(coords, i * 2);
		}
		gl2.glEnd();
	}

	private void glDrawPoint(GL2 gl2, float x, float y, float z, float size) {
		gl2.glEnable(GL2.GL_DEPTH_TEST);
	...
	private void glDrawSpiral(GL2 gl2, float radius, int rotations) {

		float dt = 1 / 6f;
		float pointSize = 2f;
		float y = 0;

		int points = rotations * 40;
		for (int i = 0; i <= points; ++i) {
			...
			if (i <= p)
				...
				gl2.glColor3ub((byte) 210, (byte) 105, (byte) 30);
			...
			glDrawPoint(gl2, x, y, z, pointSize);
			pointSize += 0.02f;
			y += 0.02;
		}
